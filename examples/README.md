# Xisp 示例和测试

本目录包含 Xisp Lisp 解释器的示例代码和测试脚本。

## 📁 文件列表

### 示例脚本

| 文件 | 说明 | 运行方式 |
|------|------|----------|
| `print_test.lisp` | 打印功能完整测试 | `./xisp examples/print_test.lisp` |
| `tutorial.lisp` | 基础教程（12个主题） | `./xisp examples/tutorial.lisp` |
| `advanced.lisp` | 高级特性展示 | `./xisp examples/advanced.lisp` |
| `quick_test.lisp` | 快速功能验证 | `./xisp examples/quick_test.lisp` |
| `bridge_test.lisp` | 仓颉桥接功能测试 | `./xisp examples/bridge_test.lisp` |
| `file_io_test.lisp` | 文件 I/O 完整测试 | `./xisp examples/file_io_test.lisp` |
| `apply_demo.lisp` | apply 函数演示 | `./xisp examples/apply_demo.lisp` |
| `apply_test.lisp` | apply 函数测试 | `./xisp examples/apply_test.lisp` |
| `destruct_test.lisp` | 解构绑定测试 | `./xisp examples/destruct_test.lisp` |

### 宏系统示例

| 文件 | 说明 | 运行方式 |
|------|------|----------|
| `macro_simple.lisp` | 简单宏演示（无反引号语法） | `./xisp examples/macro_simple.lisp` |
| `macros.lisp` | 完整宏系统（反引号语法） | `./xisp examples/macros.lisp` |
| `macro_test.lisp` | 宏系统基础测试 | `./xisp examples/macro_test.lisp` |

### 新增示例（现代化语法）

| 文件 | 说明 |
|------|------|
| `pattern_matching.lisp` | 模式匹配完整教程（含 rest 参数、守卫条件） |
| `modern_syntax.lisp` | 现代化语法特性（字面量、插值、解构、管道） |
| `destructuring.lisp` | 解构绑定深入展示 |
| `thread_macro.lisp` | 管道操作符深入展示 |
| `guard_clauses.lisp` | 守卫条件深入展示 |

## 🚀 快速开始

### 1. 运行所有测试

使用项目根目录的测试脚本：

```bash
./test.sh
```

这会自动执行：
- 编译项目
- 运行单元测试
- 运行功能验证

### 2. 运行单个示例

```bash
# 打印功能测试
./xisp examples/print_test.lisp

# 基础教程
./xisp examples/tutorial.lisp

# 高级特性
./xisp examples/advanced.lisp

# 快速验证
./xisp examples/quick_test.lisp
```

### 3. 交互式运行

```bash
./xisp
```

在 REPL 中输入：

```lisp
xisp> (println "Hello" "World")
"Hello" "World"

nil
xisp> (+ 1 2 3)

3.000000
xisp> ,exit
```

## 📚 示例说明

### print_test.lisp - 打印功能测试

测试所有 5 个内置打印函数：

- `print` - 不换行打印，参数间用空格分隔
- `println` - 打印并换行
- `princ` - Lisp 风格打印
- `display` - Scheme 风格显示
- `newline` - 输出换行符

**适用场景**: 验证打印输出功能

### tutorial.lisp - 基础教程

包含 12 个基础主题：

1. 基本算术运算
2. 变量定义
3. 比较运算
4. 逻辑运算
5. 列表基础
6. 函数定义
7. 匿名函数 (lambda)
8. 条件判断 (if)
9. 局部绑定 (let)
10. 高阶函数
11. 递归函数
12. 综合示例

**适用场景**: Lisp 初学者入门

### advanced.lisp - 高级特性

展示高级编程技巧：

1. 闭包 (Closures)
2. 函数组合 (Compose)
3. 柯里化 (Currying)
4. 列表处理管道
5. 归约操作 (Reduce)
6. 范围生成 (Range)
7. 累加器模式
8. 函数式编程

**适用场景**: 学习函数式编程

### quick_test.lisp - 快速验证

快速检查核心功能：

- 算术运算
- 变量定义
- 列表操作
- 函数调用
- 高阶函数
- 递归
- 打印

**适用场景**: 快速验证功能是否正常

### bridge_test.lisp - 仓颉桥接功能测试

演示 Lisp 如何调用仓颉标准库函数：

- `cangjie::read-file` - 读取文件内容
- `cangjie::write-file` - 写入文件
- `cangjie::exists?` - 检查文件/目录是否存在
- `cangjie::list-dir` - 列出目录内容

**适用场景**: 理解仓颉互操作桥接层

### extension_demo.cj - LispConvertible 扩展示例（仓颉程序）

展示如何使用 `LispConvertible` 接口和扩展功能：

- 基本类型转换（Int64, String, Bool）
- ArrayList/Array 转换为 Lisp 列表
- HashMap 转换为 Lisp 关联列表
- HashSet 转换为 Lisp 列表
- Option 类型转换
- 嵌套集合转换
- 便捷函数 `toLisp()` 和 `toLispList()`

**适用场景**: 学习如何在仓颉代码中使用 Lisp 类型系统

**运行方式**:
```bash
cjpm run --name "ystyle::xisp.examples.extension_demo"
```

### file_io_test.lisp - 文件 I/O 完整测试

测试真实的文件系统和文件 I/O 操作：

- `cangjie::read-file` - 读取文件内容（真实实现）
- `cangjie::write-file` - 写入文件（真实实现）
- `cangjie::append-file` - 追加内容到文件（新增）
- `cangjie::exists?` - 检查文件/目录是否存在（真实实现）
- `cangjie::file?` - 判断是否为文件（新增）
- `cangjie::directory?` - 判断是否为目录（新增）
- `cangjie::list-dir` - 列出目录内容（真实实现）

**适用场景**: 验证文件 I/O 桥接功能

**运行方式**:
```bash
./xisp examples/file_io_test.lisp
```

**特性**:
- 真实的文件读写操作
- 错误处理和异常捕获
- 文件类型判断
- 目录遍历

### macro_simple.lisp - 简单宏演示

展示不使用反引号语法的宏定义方法：

- `when-simple` - 条件执行宏
- `incf` - 变量自增宏
- `negate` - 数值取反宏
- `swap` - 交换两个变量宏

**适用场景**: 理解宏的基本工作原理

**特性**:
- 使用 `list` 和 `quote` 手工构造表达式
- 适合学习宏的展开机制

### macros.lisp - 完整宏系统演示

展示使用反引号语法的现代宏系统：

- `when` - 条件执行（支持多个 body 表达式）
- `unless` - 反向条件执行
- `let*` - 顺序绑定宏
- `cond` - 多分支条件宏
- `incf` - 自增宏（带可选参数）
- `==>` - 管道操作符宏

**适用场景**: 学习高级宏编程技巧

**特性**:
- 使用反引号（`` ` ``）和逗号（`,`）语法
- 逗号-at（`,@`）列表拼接
- `macroexpand` 和 `macroexpand-all` 宏展开演示

### macro_test.lisp - 宏系统基础测试

宏功能的快速验证测试：

- `when` 宏测试
- `unless` 宏测试
- `macroexpand` 展开测试
- `incf` 自增宏测试
- `let*` 顺序绑定测试

**适用场景**: 验证宏系统功能是否正常

## 📊 输出说明

每个表达式执行后，REPL 显示：

```
xisp> (println "Hello" "World")    <- 用户输入
"Hello" "World"                      <- 打印输出

nil                                  <- 返回值
xisp> (+ 1 2 3)                      <- 用户输入

3.000000                             <- 返回值
xisp>                                <- 下一个提示符
```

## 🔧 REPL 命令

在 REPL 中可用的特殊命令：

| 命令 | 功能 |
|------|------|
| `,help` | 显示帮助信息 |
| `,env` | 查看环境变量 |
| `,exit` | 退出 REPL |
| `,quit` | 退出 REPL |

## 💡 内置函数

### 算术
- `+` `-` `*` `/` `mod`

### 比较
- `=` `<` `>` `<=` `>=`

### 逻辑
- `and` `or` `not`

### 列表
- `list` `cons` `prepend` `first` `rest` `second` `third`
- `length` `map` `filter` `reduce` `sum` `product`
- `reverse` `range`

### 谓词
- `number?` `string?` `symbol?` `list?` `null?` `procedure?`

### 打印
- `print` `println` `princ` `display` `newline`

## 📖 更多资源

- 项目根目录: `../README.md`
- 任务追踪: `../task.md`
- 核心设计: `../docs/core.md`
- 设计文档: `../docs/design.md`
- 宏系统语法: `../docs/syntax/macros.md`

### 仓颉示例程序

如需运行仓颉示例程序（用于演示特定功能或 API 用法），请查看 `src/examples/` 目录：

```bash
# 编译所有示例程序
cjpm build

# 运行仓颉示例程序（例如模式匹配演示）
./target/release/bin/ystyle::xisp.examples.match_demo
```

可用的仓颉示例程序：
- `match_demo` - 模式匹配完整演示
- `modern_syntax` - 现代化语法演示
- `test_destruct` - 解构绑定测试
- `test_pipeline` - 管道操作符测试
- `guard_test` - 守卫条件测试
- `sandbox_demo` - 沙箱系统演示
- `options_usage` - 选项系统使用示例
- `extension_demo` - 仓颉扩展功能演示

## 🐛 问题反馈

如果发现示例有问题，请提交 issue 或 PR。

---

**版本**: 0.1.0 MVP
**最后更新**: 2026-01-23
