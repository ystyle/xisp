...
> 解释器侧的分派/cache/超指令优化已推进两轮（见第 6 节历程）；当前瓶颈转移至**函数调用非内联路径与边界场景**（高阶回调/宏等），进一步提速依赖 **JIT 特化**（类型推测 + 机器码，见 bytecode-fw 设计文档）。
...
| opcode 家族化（OP_BIN_LC + kind） | 当前 | case 数 45→42，保住跳转表分派（已实测 case 49 触发退化） |
