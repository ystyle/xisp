...
| 场景 | AST (ms) | BC (ms) | 加速 |
|---|---|---|---|
| 01 fib 直接递归 | 10799 | 1426 | **7.6x** |
| 02 fib 间接调用（闭包值经局部变量） | 10834 | 1433 | **7.6x** |
| 03 fact | 59 | 16 | **3.7x** |
| 04 尾递归求和 | 400 | 68 | **5.9x** |
| 05 列表 reduce 聚合 | 62 | 72 | 0.9x |
| 06 管道（map→filter→reduce） | 507 | 475 | 1.1x |
| 07 闭包计数器（捕获 set!） | 48 | 16 | **3.0x** |
| 08 闭包工厂（单层捕获） | 145 | 38 | **3.8x** |
| 09 match 字面量模式（5 子句） | 199 | 28 | **7.1x** |
| 10 quasiquote 构造（含 splice） | 38 | 27 | 1.4x |
| 11 let 绑定 + 算术密集 | 48 | 28 | **1.7x** |
| 12 set! 全局状态更新 | 28 | 16 | **1.8x** |
| 13 宏展开密度（when 宏） | 102 | 94 | 1.1x |
| 14 &rest 参数函数 | 59 | 71 | 0.8x |
| 15 字符串插值 | 38 | 28 | 1.4x |

**结论**：
- **函数调用密集的代码收益最大**（fib/fact/尾递归/match 5-8x）：编译消除了递归解释的表达式树遍历、环境链查找、调用栈帧管理；
- **闭包场景 3-4x**：lambda 编译为子函数 + 帧切换闭包调用；
- **边界场景持平**（高阶回调、宏展开、`&rest`、字符串插值）——见第 5 节。
...
| 场景 | 原因 |
|---|---|
| `reduce`/`map`/`filter`（高阶回调） | 编译侧不生成高阶循环，顶层表达式回退 AST；回调若为编译 lambda（Closure），经 AST `applyProcedure` 转 `Procedure(AST body)` 执行——**回调体仍在 AST 解释** |
...
**下一步优化方向**（按收益排序）：
1. 高阶回调直入 VM（`callClosure`）——让 map/filter/reduce 的回调帧切换执行；
2. `&rest`/`&key` 参数编译（参数绑定前缀代码）；
3. match guard/列表模式编译；
4. 编译缓存（同一 AST 表达式不重复编译）。
...
| 字节码初版（调用链断链） | `f57b958`~`1d3c1a5` | 仅 **~5-7%**（直接函数调用全部坠入 AST 解释——修复前实测 fib24 直接调用 429ms vs 间接 71ms） |
| **调用链修复 + 帧布局重构** | `d593c52` | 直接调用接帧切换；AS T 2038ms → **109ms（~18.7x）** @ 首版基准 |
| 合并 master AST 优化 | `3f5a953` | AST 基线 2038→~710ms；字节码 109ms（~6.5x vs 新 AST） |
| **全语法编译 + examples 双模式验证** | `1d589bb` | 本轮 15 场景数据（调用密集 5-8x，闭包 3-4x，边界持平） |
