...
## 0. 三模式性能对比（最新，2026-08-26）

Xisp 现有**三种求值模式**（CLI 切换）：

| 模式 | 启用方式 | 说明 |
|---|---|---|
| **AST 解释** | 默认 | S 表达式树递归解释；`stackDepth` 上限（默认 1000），超出返回 Error |
| **字节码 BC** | `--with-bytecode-compiler` | 顶层编译为指令流 + 寄存器栈 VM 帧切换；深递归不受 AST 栈深限制；部分语义边界回退 AST |
| **JIT 机器码** | `--with-jit`（Linux x86-64） | 字节码再翻译为 x86-64 机器码（寄存器级 R 形式 / INT 特化 / 通用三档），热点函数全机器码执行；Int/Bool 快路径，其余 deopt 回解释器 |

### 0.1 标志性基准：fib(30)（进程内，扣除 ~15ms 启动）
...
> fib(30)=832040，双递归（约 2.6M 次调用）；JIT 为寄存器级 R 形式全机器码（单测 `testJitFib30Perf` MonoTime 实测）。

### 0.2 基准套件 15 场景（`bash lisp-tests/perf/run.sh`，5 轮取中位数）

| 场景 | AST(ms) | BC(ms) | JIT(ms) | BC/AST | JIT/BC |
|---|---|---|---|---|---|
| 01 fib 直接递归 | 10893 | 1634 | 27 | 6.7x | **60.5x** |
| 02 fib 间接递归 | 10871 | 1628 | 26 | 6.7x | **62.6x** |
| 03 阶乘 | 59 | 17 | 16 | 3.5x | 1.1x |
| 04 尾递归累加 | 403 | 80 | 17 | 5.0x | **4.7x** |
| 05 列表归约 | 73 | 72 | 73 | 1.0x | 1.0x |
| 06 管道 | 618 | 482 | 478 | 1.3x | 1.0x |
| 07 闭包计数 | 50 | 16 | 118 | 3.1x | 0.1x |
| 08 闭包工厂 | 180 | 48 | 221 | 3.8x | 0.2x |
| 09 match | 201 | 27 | 216 | 7.4x | 0.1x |
| 10 quasiquote | 68 | 49 | 39 | 1.4x | 1.3x |
| 11 let 算术 | 58 | 27 | 27 | 2.1x | 1.0x |
| 12 状态修改 | 26 | 16 | 16 | 1.6x | 1.0x |
| 13 宏密度 | 111 | 112 | 154 | 1.0x | 0.7x |
| 14 rest 参数 | 59 | 70 | 70 | 0.8x | 1.0x |
| 15 字符串插值 | 37 | 39 | 39 | 0.9x | 1.0x |

### 0.3 结论与适用

- **JIT 最适合纯 Int/Bool 递归/循环密集**（fib/尾递归累加 4.7–63x）：R 形式（纯自递归 INT，含 k=2 尾递归累加器）走寄存器级全机器码。
- **BC 稳定 1–7x**，且深递归不受限（帧切换）、无平台依赖；是通用推荐档。
- **JIT 慢于 BC 的场景**（闭包/高阶回调/match/宏密度 0.1–0.7x）：这些路径走 BC VM 或 AST 内置兜底（JIT 每函数编译 + deopt 边界开销）——见各章节边界记录。
- **平台**：JIT 仅 **Linux x86-64**（经 `@When[os == "Linux" && arch == "x86_64"]` 条件编译限定：其他平台排除 mmap/机器码执行/JIT 运行时，编译进非 x86 stub → `--with-jit` 自动降级为 BC）。已用 `cjpm build --target x86_64-pc-windows-gnu` 交叉编译验证（Windows 下 JIT 正确排除、stub 生效）。BC/AST 全平台。
- **正确性**：三模式对同一脚本输出逐字节一致（examples 22/22 校验）；JIT 语义边界（Float 等类型 deopt、大常量回退、深递归栈守卫 deopt）均保真。
